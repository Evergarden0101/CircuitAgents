...
- **POMDP**。自己座標を観測に含めず、前方 27 m のエゴセントリック局所観測のみを与える。
  これにより方策クラスは平行移動・回転不変な「反射的レーン追従」に制約され、
...
- γ = 0.99 @ 5 Hz → 実効ホライズン ≈ 20 s ≈ 1 周。コーナー進入前の減速判断に対する
  信用割当が 1 周スケールで届く。GAE λ = 0.98 は密報酬前提で分散側に寄せた選択。
...
- n_steps 512 × 8 env (buffer 4096) / minibatch 256 / 8–15 epochs / 線形 LR 減衰。
- **ent_coef は報酬スケールと結合する**: 報酬を [0,1] に圧縮した本環境で 0.10 を使うと
  エントロピー項が報酬差を支配し、方策 σ が ~1.6 まで成長して実質ランダムに留まる (実測)。
  0.02 が均衡点。
- 直交初期化 + throttle バイアス +0.4 の手動シフト: ガウス方策の平均 0 では前進/後退が
  等確率で、前進報酬の発見前に壁ペナルティを蓄積する。探索の対称性を意図的に破る。
- モデル選択は EvalCallback の決定論的評価。PPO は終盤退行があり、また entropy 残存下の
  stochastic 評価は方策品質を過小評価する。

### 6.3 報酬設計と specification gaming

観測された報酬ハッキング 3 種: ①路外ショートカット (壁なし時)、
②wall-riding (反発壁が方向転換を代行)、③壁這い creeping (脱出報酬が接触時限定だった時)。

- 対策の理論的支柱は **potential-based reward shaping** (Ng, Harada & Russell 1999):
  壁クリアランスをポテンシャル φ とし、その差分に報酬を与える形は最適方策を保存する。
- 実務上は、全代表状態の per-step 報酬を実測し
  「運転 0.97 > 脱出 0.51 > 停滞 0.45 > 壁擦り 0.42」の順序をテーブルとして検証。
- dense (速度・車線中央) + sparse (チェックポイント/ラップ) のハイブリッド。
  正規化 [0,1] のクリップ飽和は milestone ステップにのみ意図的に許容。

### 6.4 sim-to-real の二層構成

1. **ギャップの除去**: 観測ビルダーは C# にビット一致、車両物理 (BicycleVehicle + RK4)
   は 1e-7 一致で移植。「移植で挙動が変わる」自由度を先に潰す。
2. **残差の吸収**: ゲーム固有の動力学差は方策を再学習せず、デプロイ側アクションフィルタ
   (EMA 平滑化・ステア速度制限・曲率ベース旋回ブレーキ・スタック検出) で吸収。
   フィルタは閉ループ特性を変えるため、シミュレータ内で同フィルタ通しの評価を行い
   周回数が維持されることを確認済み (α=0.6 で蛇行 34→12 回/100 步、周回数不変)。
