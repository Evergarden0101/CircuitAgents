...
- **Tracks**: `config["track"]` = `"fast"` (9-segment circuit a…i), `"oval"`, or `"random"` (resampled per episode — used for generalization training). Each `_make_road_*` sets `self.segment_sequence` (checkpoint lap order) and `self.spawn_options`. All tracks are 2 lanes × 5 m wide.
