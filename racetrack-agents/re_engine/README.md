...
| `CarController.cs` | Drives the car from decoded (acceleration, steering) commands using the exact training dynamics — highway-env's BicycleVehicle (linear tire slip, RK4). Owns the physics state, returns the world velocity `Vector3` each `Step`; verified bit-identical to Python over a scripted drive. |
